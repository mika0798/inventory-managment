...
public interface InventoryRepository extends JpaRepository<Inventory, Long> {
    ...
    @Query("SELECT i FROM Inventory i JOIN FETCH i.product WHERE i.store.id = :storeId AND i.product.name LIKE %:name%")
    List<Inventory> searchByNameAndStore(@Param("name") String name, @Param("storeId") Long storeId);

    List<Inventory> findByProduct_NameContainingIgnoreCaseAndStore_Id(String productName, Long storeId);

    List<Inventory> findByProduct_CategoryAndProduct_NameContainingIgnoreCaseAndStore_Id(String category,
            String productName, Long storeId);
